...
export function ContactCta() {
  return (
    ...
    >
      <div className="pointer-events-none absolute inset-x-0 bottom-0 -z-10 h-[520px] bg-[radial-gradient(ellipse_55%_60%_at_50%_100%,var(--glow-soft),transparent_72%)] blur-2xl" />
      <div className="bg-grid pointer-events-none absolute inset-0 -z-20 opacity-70" />

      <div className="mx-auto max-w-4xl px-6 text-center">
        <Reveal>
          <span className="glass text-muted inline-flex items-center gap-2 rounded-full border px-3.5 py-1.5 text-[12px] font-medium">
            <span className="relative flex h-1.5 w-1.5">
              <span className="bg-ember-400 absolute inline-flex h-full w-full animate-ping rounded-full opacity-75" />
              <span className="bg-ember-400 relative inline-flex h-1.5 w-1.5 rounded-full" />
            </span>
        ...
        <Reveal delay={0.08}>
          <h2 className="text-display gradient-text mt-8 text-[clamp(2.5rem,7vw,5rem)] font-medium text-balance">
        ...
        <Reveal delay={0.24}>
          <div className="mt-11 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <Button href={`mailto:${site.email}`} size="lg" className="w-full sm:w-auto">
              <Mail className="h-4 w-4" strokeWidth={1.35} />
              Contact Us
